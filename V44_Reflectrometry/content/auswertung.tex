\section{Auswertung}
\label{sec:Auswertung}
Alle Berechnungen werden mit dem Programm \glqq NumPy" \cite{numpy}, die Unsicherheiten mit dem Modul \glqq Uncertainties" \cite{uncertainties}, die Ausgleichsrechnungen mit dem Modul \glqq Scipy" \cite{scipy} durchgeführt und die grafischen Darstellungen über das Modul \glqq Matplotlib" \cite{matplotlib} erstellt.

\subsection{Intensitätsverteilung}

Als erstes wird die maximale Intensität $I_0$ anhand des Detektorscans bestimmt. Hier wird eine Gaußfunktion über die Curvefit Funktion aus Scipy~\cite{uncertainties}
gefittet. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/dscan.pdf}
    \caption{Detektorscan.}
    \label{fig:dscan}
\end{figure}

Die folgenden Parameter wurden somit bestimmt und die $FWHM = 2\sqrt{2\cdot\ln{2}}\cdot\sigma$ berechnet:
\begin{align}
    I_{\max} &= 228603 \;\text{s}^{-1} \\
    \mu      &= (-0.0014 \pm 0.0005)^{\circ} \\
    \sigma   &= (0.0382 \pm 0.0005)^{\circ} \\
    \text{FWHM} &= (0.0900 \pm 0.0012)^{\circ}
\end{align}
    
\subsection{Strahlbreite}

Anhand des Z-Scans kann die Strahlbreite abgeschätzt werden, indem der Bereich, in dem die gemessene Intensität stetig abnimmt, den Messdaten entnommen wird. 
In Abbildung\ref{fig:dscan} ist das entsprechende Intervall zu sehen. Die Strahlbreite ergibt sich somit zu $d = \SI{0.123}{\milli\metre}$

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/zscan.pdf}
    \caption{Zscan.}
    \label{fig:zscan}
\end{figure}

Darüber hinaus wird anhand des X-Scans die Probenbreite abgeschätzt. Dazu wird das Intervall, in dem die Intensität zwischen beiden Plateaus liegt, den Daten entnommen.
Die Probenbreite ergibt sich damit zu $D =\SI{20.0}{\milli\metre}$ und ist in Abbildung\ref{fig:xscan} visualisiert.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/xscan.pdf}
    \caption{Xscan.}
    \label{fig:xscan}
\end{figure}

Außerdem wird der Geometriewinkel $\alpha_G$ mithilfe des Rockingscans bestimmt. Die Probe wird hierbei im Strahl rotiert, wobei die Intensität gegen den Rotationswinkel
aufgetragen wird. Die beiden Grenzregionen, an denen die Intensität auf null fällt, kennzeichnen den Geometriewinkel. Dieser beträgt hier $\alpha_G = \SI{0.39}{\degree}$,
was in Abbildung~\ref{fig:rscan} visualisiert ist. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/rscan.pdf}
    \caption{Rockingscan.}
    \label{fig:rscan}
\end{figure}

$\alpha_G$ lässt sich auch rechnerisch über die bereits bestimmten Strahl- und Probenbreite anhand von Gleichung\ref{eq:breite} bestimmen. 
Damit ergibt sich $\alpha_{G_{th}} = \SI{0.3527}{\degree}$.

\subsection{Reflektivitätsmessung}

Nach Normierung der Intensität anhand der errechneten maximalen Intensität kann eine Reflektivität in Abhängikeit des Einfallswinkels der Röntgenstrahlung zur Probe 
dargestellt werden. Diese Messung wurde sowohl mit dem Detektor immer im Ausfallswinkel, als auch mit dem Detektor in einer zum Ausfallswinkel verschobenen Position 
durchgeführt. Der somit ermittelte Einfluss diffuser Streuung kann dann von der eigentlichen Messung subtrahiert werden. Dadurch ergibt sich dann die reine 
winkelabhängige Reflektivität. Beide unangepassten Verläufe sind in Abbildung~\ref{fig:ref_raw} dargestellt, sowie die angepasste Reflektivität im Vergleich zur
Fresnel-Kurve für eine optimal glatte Siliziumoberfläche in Abbildung~\ref{fig:ref_mod}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/ref_raw.pdf}
    \caption{Reflektivität und Diffuse Streuung.}
    \label{fig:ref_raw}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/ref_mod.pdf}
    \caption{reduzierte Reflektivität und Fresnel-Kurve.}
    \label{fig:ref_mod}
\end{figure}

Für kleine Winkel, für die nicht die gesamte Breite des Strahls die Probe trifft, wird die Reflektivität durch den Geometriefaktor aus Gleichung~\ref{eq:g_factor} korrigiert.
In Abbildung~\ref{fig:g_corr} ist die korrigierte Reflektivität für Winkel $ \alpha_g < \SI{0.3527}{\degree}$ dargestellt.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/g_corr.pdf}
    \caption{Reflektivität mit und ohne Geometriefaktorkorrektur.}
    \label{fig:g_corr}
\end{figure}

Mit der korrigierten Reflektivität werden alle weiteren Auswertungsschritte durchgeführt. Zunächst wird die Schichtdicke des Polystyrol-Films über die Gleichung~\ref{eq:breite} 
abgeschätzt. Dabei wurde $ q_z = \frac{4\pi}{\lambda}\sin(\alpha) $ benutzt, um im $q$-Raum die Amplituden der Kiessig-Oszillationen zu finden. Der
Wellenvektorübertrag $\vec q = \vec k_{\mathrm{out}} - \vec k_{\mathrm{in}}$ beschreibt
die Impulsänderung der Photonen beim Streuprozess, wobei $\vec k_{\mathrm{in}}$ und
$\vec k_{\mathrm{out}}$ die ein- bzw. auslaufenden Wellenvektoren mit Betrag $k = \frac{2\pi}{\lambda}$
sind. Die Messung erfolgt hierbei nur mit gleichem Ein- und Ausfallswinkel, weshalb keine laterale Impulsübertragung stattfindet und nur die
senkrechte Komponente $q_z$ zur Streuung beiträgt. Dies wurde mit der 
Scipy Funktion FindPeaks durchgeführt. Die ermittelten Amplituden sind in Abbildung~\ref{fig:peaks} zu sehen. Mit dem so bestimmten mittleren Amplitudenabstand 
$\Delta q_z = 0.0071\pm0.0005$ kann die Schichtdicke zu $d = \SI{88.27\pm6.24}{\nano\metre}$ bestimmt werden, was eine erste Abschätzung darstellt. 
Hierbei wurde bewusst nur der Winkelbereich gewählt, in dem klare Kiessig-Oszillationen sichtbar sind. Außerdem wurde an der $\alpha > \SI{0.6}{\degree}$ 
der Plot für diesen Zweck abgeschnitten, da der große Peak zwischen $\SI{0.6}{\degree}$ und $\SI{0.8}{\degree}$ keine Kiessig-Oszillation dargestellt, siehe 
Abschnitt~\ref{sec:Diskussion}, und dieser Bereich die Periodizität der Oszillationen signifikant verändert. Dies erhöht die Varianz des berechneten $d$ zu 
unbrauchbaren Werten. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/kiessig.pdf}
    \caption{Amplituden der Kiessig Oszillationen für $ \SI{0.6}{\degree} < \alpha < \SI{0.8}{\degree}$.}
    \label{fig:peaks}
\end{figure}

Wie im Unterabschnitt~\ref{sec:multi} beschrieben, können mit dem Parratt-Algorithmus weitere Eigenschaften der Probe anhand der Reflektivitätsmessung bestimmt werden.
Dabei wurden für das Siliziumsubstrat jeweils der Dispersions- und Absorptionskoeffizient auf Literaturwerte fixiert, da Silizium ein Referenzmaterial mit bekannten 
optischen Konstanten ist. Das Übergeben dieser Größen als Fit-Parameter würde keine zusätzlichen physikalischen Informationen liefern und außerdem die Stabilität des 
Parratt-Fits verringern. \newline
Die durch den Fit bestimmten Parameter sind die Schichtdicke des Polystyrolfilms $d_\text{poly}$, die Dispersions- und Absorptionskoeffizienten des Polystyrols 
$\delta_\text{poly}$ und $\beta_\text{poly}$, sowie die Rauigkeiten $\sigma_\text{poly}$ und $\sigma_\text{sili}$ an beiden Grenzflächen.
Aus den Dispersionskoeffizienten wurden außerdem jeweils die kritischen Winkel anhand von Gleichung~\ref{eq:alpha_c} berechnet.\newline
Darüber hinaus wurden als instrumentelle Korrekturparameter ein Winkel-Offset $\alpha_0$, sowie ein Skalierungsfaktor $S$ eingeführt. Durch $\alpha_0$ können kleine
Justagefehler, die zu einer 
systematischen Winkelverschiebung führen, ausgeglichen werden. Der Skalierungsfaktor wirkt multiplikativ auf die durch den Fit bestimmte Reflektivität. Dies kann 
mögliche Fehler in der Normierung der Primärintensität $I_0$ ausgleichen. Die hierüber bestimmten Parameter des Parratt-Fits sind in der Tabelle~\ref{tab:parratt}
angegeben.

\begin{table}[h!]
    \centering
    \caption{Ergebnisse des Parratt-Fits für den Polystyrolfilm auf Silizium, inkl. fixierter Parameter.}
    \label{tab:parratt}
    \begin{tabular}{l c c}
        \hline
        Parameter & Wert & Einheit \\
        \hline
        Schichtdicke $d$ & $90.40$ & nm \\
        Dispersion Polystyrol $\delta_{\mathrm{Pol}}$ & $2.11 \times 10^{-6}$ & -- \\
        Dispersion Silizium $\delta_{\mathrm{Si}}$ & $7.60 \times 10^{-6}$ (fixiert) & -- \\
        Kritischer Winkel Polystyrol $\alpha_{c,\mathrm{Pol}}$ & $0.118$ & $^\circ$ \\
        Kritischer Winkel Silizium $\alpha_{c,\mathrm{Si}}$ & $0.223$ & $^\circ$ \\
        Rauigkeit Luft/Polystyrol $\sigma_{01}$ & $21.67$ & \AA \\
        Rauigkeit Polystyrol/Silizium $\sigma_{12}$ & $5.41$ & \AA \\
        Absorption Polystyrol $\beta_{\mathrm{Pol}}$ & $3.81 \times 10^{-28}$ & -- \\
        Absorption Silizium $\beta_{\mathrm{Si}}$ & $1.90 \times 10^{-7}$ (fixiert) & -- \\
        Winkeloffset $\alpha_0$ & $-0.0099$ & $^\circ$ \\
        Skalierungsfaktor $S$ & $0.5313$ & -- \\
        \hline
    \end{tabular}
\end{table}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/parratt.pdf}
    \caption{Reflektivität mit Parratt Fit.}
    \label{fig:parratt}
\end{figure}

% \begin{table}[h]
%     \centering
%     \caption{Parratt-Fit Parameter im Vergleich zu Theoriewerten.}
%     \label{tab:parratt_theory}
%     \begin{tabular}{l c c}
%     \toprule
%     \textbf{Parameter} & \textbf{Parratt Algorithmus} & \textbf{Theoriewert} \\
%     \midrule
%     $\delta_{\mathrm{Si}}$      & $7.971 \cdot 10^{-6}$ & $7.6 \cdot 10^{-6}$ \\
%     $\delta_{\mathrm{Pol}}$     & $2.189 \cdot 10^{-6}$ & $3.5 \cdot 10^{-6}$ \\
%     $\sigma_{\mathrm{Si}}$      & $15.13\,\AA$          & $10\,\AA$ \\
%     $\sigma_{\mathrm{Pol}}$     & $23.31\,\AA$          & $10\,\AA$ \\
%     $\beta_{Si}$                 & $2.227 \cdot 10^{-7}$ & $1.9 \cdot 10^{-7}$ \\
%     $\beta_{Pol}$                 & $1.019 \cdot 10^{-7}$ & $1.75 \cdot 10^{-8}$ \\
%     $d$                         & $87.80\,\mathrm{nm}$  & $8.9 \cdot 10^{-8}\,\mathrm{m}$ \\
%     \bottomrule
%     \end{tabular}
%     \label{tab:parratt}
%     \end{table}
    

