...
\newline\newline
Mit $D =\SI{20.0}{\milli\metre}$ beträgt die durch den X-Scan ermittelte Probenbreite exakt den selben Wert, wie in der Versuchsanleitung~\cite{V44} angegebenen.
...
bis zu einer klaren Kante bei $\alpha \approx \SI{0.2}{\degree}$, was konsistent mit $\alpha_{c,\mathrm{Si}} =0.223 ^\circ$ ist. Ab hier fällt die Reflektivität ab, zeigt aber 
...
der Literatur für Cu-K$\alpha$-Strahlung typischerweise im Bereich $\delta_\text{Poly} = (2{-}3) \times 10^{-6}$, wie in Quelle~\cite{Henke} zu finden ist.
In der Versuchsanleitung~\cite{V44} ist außerdem noch der exakte Wert $\delta_\text{Poly} = 3.5 \times 10^{-6}$ angegebenen. Damit
...
dem Verlauf der Messwerte übereinstimmt. Die kritischen Winkel, die direkt über Gleichung~\ref{eq:alpha_c} bestimmt wurden, geben hier keine weitere
direkte Information. Ein Vergleich mit den in der Versuchsanleitung angegebenen Werten zeigt allerdings, dass die Werte jeweils in der richtigen Größenordnung liegen, 
jedoch numerisch signifikant abweichen (siehe Tabele~\ref{tab:krit}).
\begin{table}[h!]
    \centering
    \caption{Vergleich der aus dem Parratt-Fit bestimmten kritischen Winkel mit den Literaturwerten aus der Versuchsanleitung.}
    \label{tab:krit}
    \begin{tabular}{l c c c}
        \hline
        Material & $\alpha_c$ (Fit) [$^\circ$] & $\alpha_c$ (Literatur) [$^\circ$] & Abweichung [\%] \\
        \hline
        Polystyrol & 0.118 & 0.153 & $-22.9$ \\
        Silizium   & 0.223 & 0.174 & $+28.2$ \\
        \hline
    \end{tabular}
\end{table}
