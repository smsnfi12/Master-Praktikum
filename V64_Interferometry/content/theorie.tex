...
A light field is called coherent when there is a stable, predictable phase relationship between the electric field at different locations or at different times \cite{RP_Coherence}.
%Coherence refers to the correlation between the phases of two waves. 
Two waves are coherent if they have a constant phase relationship, same frequency, and polarization.
...
Polarization of the electromagnetic wave describes the geometrical orientation of the wave's electric field vector $\vec{E}(t)$. 
For a plane wave propagating in direction $\vec{k}$, the electric-field vector $\vec{E}$ is transverse, so the polarization state is defined by the trajectory traced by $\vec{E}$ in the plane perpendicular to the direction of propagation.
Light waves can be linearly, circularly, or elliptically polarized.
Linear polarization describes oscillations along a fixed direction in the transverse plane, while circular polarization corresponds to a rotation of $\vec{E}$ at constant magnitude.
In elliptical polarization, the tip of the electric field vector describes an ellipse.\\
The optical plane is the plane spanned by the propagation direction of the light wave and the surface normal of an interface, such as a mirror or a beam splitter.
%For reflections and refractions at interfaces, the optical plane is defined by the incident wave vector and the surface normal.
...
The polarization angle $\phi$ is the angle between the direction of the wave's electric field oscillation and a chosen reference axis in the plane perpendicular to the propagation direction.
It is natural to choose the reference as the s-direction. That means $\phi = 0^\circ$ corresponds to s-polarization, and $\phi = 90^\circ$ corresponds to p-polarization.
...
\begin{align}
    \Delta \delta (\Theta) &\approx \frac{2\pi}{\lambda_\text{vac}} T \frac{n - 1}{2n} \left( (\Theta_1+\Theta)^2 - (\Theta_2-\Theta)^2 \right) \nonumber \\
    & = \frac{2\pi}{\lambda_\text{vac}} T \frac{n - 1}{n} 2 \Theta_1 \cdot \Theta.
\end{align}
...
\begin{equation}
    M = \frac{\Delta \delta}{2\pi} = \frac{T}{\lambda_\text{vac}} \frac{n - 1}{2n} 4 \Theta_1 \cdot \Theta.
    \label{eq:glas_index}
\end{equation}
So the refractive index of the glass can be obtained from equation \eqref{eq:glas_index} as
\begin{equation}
    n = \frac{1}{1 - \frac{M \lambda_\text{vac}}{2 T \Theta_1 \Theta} }.
...
With the molar refractivity $A$ and the ideal gas law $pV = nRT$, where $p$ is the pressure, $V$ the volume, $n$ the amount of substance, $R$ the ideal gas constant, and $T$ the temperature, the equation \eqref{eq:lorentz_lorenz} becomes
\begin{equation}
    \frac{n^2 - 1}{n^2 + 2} = \frac{A p}{R T}.
\end{equation}
For small refractive indices ($\Delta n$ << 1), the refractive index can be approximated as
\begin{equation}
    n \approx 1 + \frac{3 A p}{2R T}.
