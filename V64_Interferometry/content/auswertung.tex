...
the mean values at each angle, the contrasts $V_\text{exp.}(\phi_\text{i})$ were calculated via equation [??]. The results are visualized in figure~\ref{fig:contrast_plot}
alongside an idealized theory distribution of $V_\text{theo.}(\phi)$ from equation [??]. 
\begin{figure}[h!]
    \centering
...
For the determination of the refractive indey of Glass, a pair of rotating glass plates, described in section [??], with thickness $d= \SI{1}{\milli\meter}$,
...
\newline A direct connection between the refractive index $n_\text{glass}$ and the mean count of interference fringes $M$ is layed out in equation [??]. The 
...
As depicted in table~\ref{tab:gas_counts}, 4 runs of interference fringe counts for a series of air pressure values were recorded. Via equation [??], the count 
...
\end{equation}
According to equation [??], the gradient $m$ is given by 
\begin{equation}
    m= \frac{3A}{2RT}\,,
\end{equation}
