...
Das Übertragen niederfrequenter Strahlung über lange Reichweiten ist in der Praxis nicht ungeeignet, da das Empfangen nur über Antennen möglich ist, 
...
Eine der möglichen Methoden, ein Trägersignal zu modulieren, ist die Amplitudenmodulation\,(AM). Durch eine geeignete Schaltung wird eine modulierte Spannung gemäß 
\begin{equation}
    u_{\mathrm{AM}}(t)
    =
...
Durch das Produkt der beiden zeitabhängigen Terme ergibt sich ein charakteristisches Spektrum. Der Term $ U_T \cos(\omega_T t)$ erzeugt die Trägerlinie bei 
...
\begin{equation}
    ...
    \label{eq:modulationsgrad}
\end{equation}
bestimmt werden. 
Alternativ kann der Modulationsgrad auch im Frequenzraum bestimmt werden. Aus dem Verhältnis der Seitenband- zur Trägeramplitude kann $m$ über
\begin{equation}
    m = 2\,\frac{U_{\mathrm{SB}}}{U_T}
    \label{eq:mod_2}
\end{equation}
ausgedrückt werden.
\newline\newline
...
\end{equation}
mit dem FM-Modulationsindex $\beta = \frac{\Delta\omega}{\omega_M}$, der das Verhältnis von maximalem Frequenzhub zur Modulationsfrequenz angibt.\newline
...
Um experimentell eine FM zu erzeugen, kann eine Schaltung mit einem Ringmodulator wie in Abbildung~\ref{fig:fm} verwendet werden. Dabei wird das Trägersignal in 
zwei Pfaden aufgeteilt, von denen einer um $90^\circ$ phasenverschoben wird. Dieser Teil wird dann mit dem Signal am Ausgang des Ringmodulators überlagert, sodass 
sich ein Gesamtsignal der Form $u(t) = A \sin(\omega_T t) + B(t)\cos(\omega_T t)$ mit $B(t)\propto U_M(t)$ ergibt. Durch die Zeitabhängigkeit von $B(t)$ lässt sich 
der Ausdruck auch als 
\begin{equation}
    u(t) = C(t) \sin\bigl( \omega_T t + \phi(t) \bigr)
\end{equation}
schreiben, wobei $C(t)$ nahezu konstant ist. Die zeitabhängige Phase $\phi(t)$ führt dann zu einer Frequenzmodulation des Trägersignals.

\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{data/fm.png}
    \caption{Skizze einer FM über einen Ringmodulator.}
    \label{fig:fm}
\end{figure}

\subsection{Demodulation}
\label{sec:demodulation}
Die Demodulation ist die Rückgewinnung des Modulationssignals aus dem modulierten Trägersignal. Bei der AM muss dafür die Hüllkurve des modulierten Signals 
extrahiert werden. Dazu kann eine Diode verwendet werden, die nur den positiven Anteil des Signals durchlässt. Anschließend wird das Signal über einen Tiefpass 
gefiltert, um die hochfrequenten Anteile zu entfernen und als Resultat das reine Modulationssignal zu erhalten. \newline
Eine weitere Möglichkeit ist die synchrone Demodulation. Der Vorteil dieser Methode ist die höhere Störsicherheit, da das Demodulationssignal phasengleich 
zum Trägersignal sein muss. Dadurch können Störungen, die nicht phasengleich sind, effektiv unterdrückt werden. Bei der AM wird das modulierte Signal erneut mit dem 
Trägersignal multipliziert.
Dadurch entstehen wieder die Frequenzanteile bei $\omega_T$ und $(\omega_T \pm \omega_M)$. Durch anschließendes Tiefpassfiltern wird das Modulationssignal extrahiert.
\newline
Zur Demodulation eines FM Signals wird dieses zuerst in ein AM Signal umgewandelt. Dazu wird das FM Signal an einen frequenzabhängigen Schwingkreis 
gekoppelt, dessen Übertragungsfunktion im Bereich der Trägerfrequenz eine näherungsweise lineare Frequenzabhängigkeit aufweist, wie in Abbildung~ß\ref{ig:fm_convert}
zu sehen ist. Dadurch entsteht ein AM Signal, welches anschließend wie oben beschrieben demoduliert werden kann.
\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{data/fm_convert.png}
    \caption{Frequenzabhängigkeit der Übertragungsfunktion eines Schwingkreises zur Umwandlung von FM in AM.}
    \label{ig:fm_convert}
\end{figure}
