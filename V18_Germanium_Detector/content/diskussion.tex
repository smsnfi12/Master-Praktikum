...
\subsection{Full Energy Probability}
...
The figure \ref{fig:Full_energy_probability} shows that some data points deviate significantly from the fit function, which could be a reason for the large uncertainties.


\subsection{FWHM and FWTM}
...
\end{equation*}
Unfortunately the measured area under the Compton continuum und the full energy peak does not yield a similar ratio
\begin{equation*}
    \frac{A_\text{Compton}}{A_\text{Full}} = 2,62~.
\end{equation*}
...
\subsection{Activity Measurement}
...
It is also important that the detection probability $Q$ for each peak, was calculated using the fit function from figure \ref{fig:Detection_probability}.
