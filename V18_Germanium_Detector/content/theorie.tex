...
\end{equation}
The Compton effect is a process in which only part of a photon's energy is absorbed. Here, the photon transfers a portion of its energy to a quasi-free 
...
\begin{equation}
    ...
    = 
      ...
      \right],
\end{equation}\,,
with $\varepsilon = \frac{E}{m_e c^2} $. Figure~\ref{fig:klein} depicts the $theta$-dependency of the cross section. In $\phi$ direction, it is constant.
\begin{figure}[h!]
    \centering
...
of the electron and the positron. This momentum can be absorbed by a significantly heavier nucleus at negligable energy cost, in which case the 
...
than that of the photoeffect and Compton scattering. This means that its contribution is negligable for this experiment's purposes.\newline

A visualization of the interaction processes dominating different photon energy regions is shown in figure~\ref{fig:att}. The decomposed linear attenuation 
coefficient $\mu$ is plotted on the y-axis. It represents the probability per unit path length that a photon is either absorbed or scattered via one of the 
interaction mechanisms discussed above. The attenuation coefficient is related to the photon beam intensity through
\begin{equation}
I(z) = I_0, e^{-\mu z},.
\end{equation}
In the relevant energy range of this experiment, and neglecting pair production, Compton scattering is the dominant interaction process for photon energies above 
approximately \SI{150}{\kilo\electronvolt}. However, since the photoelectric effect exhibits a strong dependence on the atomic number ($\sigma_{\mathrm{ph}} 
\propto Z^{n}$ with $n \approx 4$–$5$), and germanium has a comparatively high atomic number of $Z = 32$,
the photoelectric effect still contributes significantly. This allows for the formation of well-defined full-energy peaks in the measured $\gamma$-spectrum.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{data/attenuation_coeff.png}
    \caption{Attenuation coefficients of the contributing interactions as a function of photon energy.~\cite{kolanoski_wermes_detektoren_2016}}
    \label{fig:att}
\end{figure} 
\newpage
...
\begin{figure}[h!]
    ...
    \label{fig:pr_spec}
\end{figure} 
The decay of a radioactive isotope can leave the daughter nucleus in an excited state, which subsequently de-excites by emitting one or more $\gamma$ photons with well-defined energies.
 Depending on the specific isotope, its decay product can remain in an 
...
a more detailed analysis of its spectrum. An element with this characteristic is $^{137}\mathrm{Cs}$, for which the energy spectrum is depicted in 
...
The counts at higher energies can be explained by a 'pulse pile-up' where 
...
Compton scattering instead, only part of its energy is absorbed by an electron. The Compton edge describes the maximum energy transfer from a photon to an 
...
\begin{equation}
    P(d) = 1 - e^{-\mu d}\,.
    \label{eq:absorption Probability}
\end{equation} 
...
Here, $A$ is the source activity, $\omega_\gamma$ is the branching ratio of the $\gamma$-line, $t$ is the measuring time, and $\Omega$ is the solid angle that the 
detector encompasses. $\Omega$ can be calculated knowing the parameters of the experiment's setup. In this case, it is given by 
\begin{equation}
...
bands overlap, while in an insolater, the energy gap is large. In a semiconductor, the energy separation between the valence band and the conduction band is 
sufficiently small that electrons can be excited from the valence band into the conduction band by either thermal excitation or external energy deposition, 
the latter being relevant in this experiment. Such an excitation leaves behind a positively charged electron hole in the valence band. The resulting electron–hole 
pair acts as a pair of mobile charge carriers within the material. When a photon deposits its energy
...
exist without exitation. For this purpose, two heavily doped regions, denoted n$^+$ and p$^+$, are created. The n$^+$ contact is formed by lithium diffusion, 
providing an excess of electrons and a conductive contact, while the p$^+$ contact is realized as an evaporated gold layer, which likewise serves as a conductive 
contact and establishes a region with a high hole concentration. By applying a sufficiently large reverse-bias voltage across these contacts, a fully depleted region 
is created within the semiconductor, effectively removing free charge carriers from the active volume.

If a photon interacts within this depleted region and creates an electron–hole pair, the electrons drift towards the n$^+$ contact, while the holes drift towards 
the p$^+$ contact under the influence of the electric field. This charge motion induces a transient current signal, which can be measured and is proportional to the 
number of created electron–hole pairs and therefore to the deposited energy.
