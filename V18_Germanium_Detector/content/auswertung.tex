...
All calculations are performed using the program \glqq Numpy" \cite{numpy}, the uncertainties are handled with the \glqq Uncertainties" module \cite{uncertainties}, the fitting procedures are carried out with the \glqq Scipy" module \cite{scipy}, and the graphical representations are created using the \glqq Matplotlib" module \cite{matplotlib}.
...
The measurement time for the $^{152}$Eu source is $t_{\text{Eu}} = 6266$ s and for the background $t_{\text{bkg}} = 75671$ s.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/Eu_spectrum.pdf}
    \caption{Spectrum of the $^{152}$Eu source and the background normalized by the time.}
    \label{fig:Energy_calibration}
\end{figure}

\subsubsection{Energy Calibration}
\label{sec:Energy_calibration}
To calibrate the energy scale of the germanium detector, the background is subtracted from the measured spectrum of the $^{152}$Eu source.
...
A Gaussian fit is performed on the peaks as shown in figures \ref{fig:Peak1_fit} and \ref{fig:Peak7_fit} for peak 1 and peak 7, respectively.
...
\begin{figure}[H]
    \centering
        \centering
        \includegraphics[width=0.7\textwidth]{plots/Eu_peak1.pdf}
...
\begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{plots/Eu_peak7.pdf}
...
The peaks in the figures above show the normalized counts per second for each channel. 
...
The energy values $E_\text{lit}$ and their probabilities $P_\text{lit}$ are taken from \cite{eu152}.
The table \ref{tab:livetimes} lists the energies with the highest probability of emission for $^{152}$Eu, their intensities, the corresponding channel numbers and the determined peak areas.\\
...
\begin{align*}
    Q(E) & = 29,69 \cdot E^{\prime ~ -0,82} \pm E^{\prime ~ -0,82} \cdot \sqrt{142,33 + 5,64 \cdot \ln^2(E^{\prime})}\\
    & = 29,69 \cdot E^{\prime ~ -0,82} \left( 1 \pm (0,1615+0,0064 \cdot \ln ^2 E^{\prime} ) \right),
\end{align*}
where the uncertainty of $Q$ is calculated by Gaussian error propagation.
...
\begin{figure}[H]
    ...
    \caption{Spectrum of the $^{137}$Cs source and the background normalized by the time.}
...
FWHM and FWTM can be determined by the Gaussian fit parameters.\\
...
%     N_{\text{FWTM}} & =\frac{N_\text{max}}{10} = 100,3~.
% \end{align*}
...
Peak positions are determined from the data. Due to discrete binning and the ambiguity in selecting the peak center, the channel uncertainties are estimated to be approximately $10$ channels.
...
\end{align*}
The determined Compton continuum and the backscatter peak are shown in figure \ref{fig:Cs_peak_data}.
...
The differential cross section of the Compton scattering is described by the Klein-Nishina formula \ref{eq:Klein-Nishina}, which can be rewritten as
\begin{equation}
    \frac{\text{d}\sigma}{\text{d}\Omega} = \frac{3}{8} \sigma_\text{Th} \frac{1}{m_0 c^2 \epsilon^2} \cdot \left( 2 + \left(\frac{E}{E_\gamma -E}\right)^2 \left[\frac{1}{\epsilon^2}+ \frac{E_\gamma - E}{E_\gamma} -\frac{2}{\epsilon} \frac{E_\gamma-E}{E}\right] \right).
\end{equation}
To determine the area under the Compton continuum, the Klein-Nishina formula is fitted to the data between the Compton edge $N_\text{Compton} = 3912$ and the backscatter peak $N_\text{Backscatter} = 1618$ as shown in figure \ref{fig:Cs_compton_fit}.
The constant factors in the Klein-Nishina formula are combined into a single fit parameter $c$ to perform the fit.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{plots/Cs_curve.pdf}
    \caption{Linear fit of the Compton continuum of the $^{137}$Cs source.}
    \label{fig:Cs_compton_fit}
\end{figure}
The resulting fit parameter $c$ is determined to be
\begin{equation*}
    c = (-6,32 \pm 0,06) \cdot 10^{-6}.
\end{equation*}
% \begin{align*}
...
\begin{figure}[H]
    ...
    \caption{Spectrum of the $^{133}$Ba source normalized by the time and the background.}
...
The spectrum of the unknown source deprived of the normalized background contribution by the measurement time is shown in figure \ref{fig:Ba_peaks}.
...
\begin{figure}[H]
    ...
    \caption{Identified peaks of the unknown source.}
    \label{fig:Ba_peaks}
\end{figure}
...
$^{214}$Pb and $^{214}$Bi belong to the $^{238}$U decay series.
$^{234}$Th is the first decay product of $^{238}$U and decays into $^{226}$Ra, which then decays into $^{214}$Pb and $^{214}$Bi.
